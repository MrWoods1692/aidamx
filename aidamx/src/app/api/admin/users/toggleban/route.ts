...
export async function POST(request: Request) {
  try {
    ...
    if (ban === undefined) {
      return NextResponse.json({ success: false, message: '操作类型未指定' }, { status: 400 });
    }
    ...
    try {
      ...
      console.log('检查is_banned字段结果:', hasBannedField ? '字段已存在' : '字段不存在', tableInfoArray);
      ...
      if (!hasBannedField) {
        ...
        try {
          ...
          console.log('已向users表添加is_banned字段，添加成功');
        } catch (alterError: any) {
          console.error('添加is_banned字段失败:', alterError.message);
          // 继续执行，因为可能是字段已存在导致的错误
        }
      }
